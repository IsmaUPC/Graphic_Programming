...
void Init(App* app)
{
   ...
   // app->worldMatrix = glm::mat4(0);
	...
	for (int i = 0; i < attributeCount; i++)
	{
		...
		texturedMeshProgram.vertexInputLayout.attributes.push_back(VertexShaderAttribute(attributeLocation, GetComponentCount(attributeType)));
	}
...
void Render(App* app)
{
	...
	case Mode_Mesh:
	{
		...
		{
			Model& model = app->models[app->entities[j].modelIndex];
			...
			for (u32 i = 0; i < mesh.submeshes.size(); ++i)
			{
				...
				glBindBufferRange(GL_UNIFORM_BUFFER, BINDING(1), app->bufferHandle, app->entities[j].localParamsOffset, app->entities[j].localParamsSize);
